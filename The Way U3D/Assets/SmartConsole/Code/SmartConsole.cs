...
public class SmartConsole : MonoBehaviour
{
	...
	void Update()
	{
		...
		if( s_first )
		{
			...
			SetTopDrawOrderOnGUIText( s_fps.GetComponent<UnityEngine.UI.Text>() );
			SetTopDrawOrderOnGUIText( s_textInput.GetComponent<UnityEngine.UI.Text>() );
			foreach( GameObject historyLine in s_historyDisplay )
			{
				SetTopDrawOrderOnGUIText( historyLine.GetComponent<UnityEngine.UI.Text>() );
			}
		...
		if( ( s_textInput != null )
		   && ( s_textInput.GetComponent<UnityEngine.UI.Text>() != null ) )
		{
			s_textInput.GetComponent<UnityEngine.UI.Text>().text = ">" + s_currentInputLine + ( ( s_blink ) ? "_" : "" );
		}
		...
		if( s_drawFPS )
		{
			s_fps.GetComponent<UnityEngine.UI.Text>().text = "" + ( 1.0f / Time.deltaTime ) + " fps ";
	...
	{
		return AddChildWithComponent< UnityEngine.UI.Text >( name );		
	}
	...
	private static void SetTopDrawOrderOnGUIText( UnityEngine.UI.Text text )
	{
	...
	private static void LayoutTextAtY( GameObject o, float y )
	{
		...
		o.GetComponent<UnityEngine.UI.Text>().fontStyle = FontStyle.Normal;
		o.GetComponent<UnityEngine.UI.Text>().font = s_font;
	}
	...
	private static void SetStringsOnHistoryElements()
	{
		for( int i = 0; i < k_historyLines; ++i )
		{
			...
			if( historyIndex >= 0 )
			{
				s_historyDisplay[ i ].GetComponent<UnityEngine.UI.Text>().text = s_outputHistory[ s_outputHistory.Count - 1 - i ];
			}
			else
			{
				s_historyDisplay[ i ].GetComponent<UnityEngine.UI.Text>().text = "";
			}
		}
	}
