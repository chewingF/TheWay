...
namespace Invector.CharacterController
{
    public abstract class vThirdPersonMotor : MonoBehaviour
    {
        ...
        [Tooltip("The extra jump speed modify if character is sprinting .")]
        public float fowardDiff = 1.3f;
        ...
        {
			//Debug.Log ("tarck: isJumping = " + isJumping);//from Harry
            CheckGround();
        ...
        public void AirControl()
        {
            ...
            //By Aubrey, change jump distance depend on if player is sprinting or not
            var jumpSpeed = speed;
            var jumpDirection = direction;
            if (isSprinting)
            {
                jumpSpeed = speed * fowardDiff;
                jumpDirection = direction * fowardDiff;
            }
            var velY = transform.forward * jumpForward * jumpSpeed;
            velY.y = _rigidbody.velocity.y;
            var velX = transform.right * jumpForward * jumpDirection;
            velX.x = _rigidbody.velocity.x;
            ...
            if (jumpAirControl)
            {
                if (isStrafing)
                {
                    ...
                    var vel = transform.forward * (jumpForward * jumpSpeed) + transform.right * (jumpForward * jumpDirection);
                    _rigidbody.velocity = new Vector3(vel.x, _rigidbody.velocity.y, vel.z);
                }
                else
                {
                    var vel = transform.forward * (jumpForward * jumpSpeed);
                    _rigidbody.velocity = new Vector3(vel.x, _rigidbody.velocity.y, vel.z);
                }
            }
            else
